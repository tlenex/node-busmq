...
// BGR: replaced by driver.js
// Bus.prototype.setupDriver = function(driver) {
//
//   if (typeof driver === 'function' && driver.name === 'Redis') {
//     this.driver = {
//       name: 'ioredis',
//       create: function(port, host, password) {
//         return new driver({
//           port: port,
//           host: host,
//           password: password,
...
//       name: 'redis',
//       create: function(port, host, password) {
//         var client = driver.createClient(port, host, {auth_pass: password});
...
//
//   this.logger.isDebug() && this.logger.debug('using driver "' + this.driver.name + '"');
// };
...
Bus.prototype.connect = function() {
  ...
  var _this = this;
  var directConnections = this.options.redis || this.options.ioredis;
...
function createConnection(index, bus, readyCallback, endCallback) {
  ...
  var _onConnectionMessage = function (message) {
    if (bus.eventsCallbacks[message.channel]) {
      bus.eventsCallbacks[message.channel]('message', message.channel, message.message);
    }
  };
...
Bus.prototype._online = function() {
        ...
        connection.script('load', script.content, function(err, resp) {
          ...
          if (--dones === 0) {
            ...
            if (_this.options.layout === 'cluster') {
              //unfortunately in a cluster, scripts tend to take a bit to be actually loaded
              _this._waitForScripts(connection, function() {
                _this.emit('online');
              });
            }
            else {
  ...
  Bus.prototype._waitForScripts = function(connection, done) {
    var _this = this;
    var hashes = Object.keys(this.scripts).map(function(key){ return _this.scripts[key].hash;});
    connection.script('exists', hashes, function(err, res) {
      if (res.length !== hashes.length || res.filter(function(r) { return r === 0 }).length === 0) {
        done();
      } else {
        console.log('XXXXXXXSCRIPTS NOT ALL THERE: ', res);
        setTimeout(function() {_this._waitForScripts(connection, done);}, 500);
      }
    });
  };
...
Bus.prototype._subscribe = function(connection, channels, cb) {
  ...
  connection.subscribe(channels, function(err) {
    ...
    if (_this.driver.name === 'ioredis') {
      // ioredis does not emit the 'subscribe' event so need to imulate node-redis behaviour
      // because this event with trigger the initial consumption of queue messages by calling consumeMessages()
      channels.forEach(function(c) {
        connection.emit('subscribe',c);
      });
    }
  });
};
