...
var useIORedis = process.env.BUSMQ_USE_IOREDIS === "yes" || process.env.BUSMQ_USE_IOREDIS === "true";
var redis = useIORedis ? require('ioredis') : require("redis");
...
Bus.prototype._subscribe = function(connection, channels, cb) {
  ...
  connection.subscribe(channels[0], function(err, ioRedisCount /* only IORedis publishes the count - node-redis emits a 'subscribe' event with the details instead */) {
    if (err) {
      _this.emit('error', "error subscribing to channels " + channels + ": " + err);
      return;
    }
    if (_this.useIORedis) {
      //BGR: IORedis does not emit the 'subscribe' event
      // Simulate node redis behaviour because this event with trigger the initial consumption of queue messages by calling consumeMessages()
      // Rather than relying on this event, maybe we could directly pass 'subscribe" to the callback 'cb' after
      // having check there is no error. This should work for both node-redis and ioredis
      channels.forEach(function(c) {
        connection.emit('subscribe',c, ioRedisCount);
      });
    }
  });
};
