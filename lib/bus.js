 ...
 * @param urls string url of a single redis to connect to or an array of string urls to connects to
 */
Bus.prototype.connect = function(urls) {
  ...
  this.urls = urls;
  this.logger.debug("bus connecting to: " + this.urls);
  ...
  urls.forEach(function(url, index) {
    if (url.indexOf('redis:') === 0) {
      ...
      connection.on("end", function (err) {
        if (--readies === 0) {
          _this._offline(url, err);
        }
      });
      connection.on("error", function (err) {
        //_this.logger.debug("received error: " + err);
 ...
 */
Bus.prototype._offline = function(url, err) {
  this.online = false;
