...
/**
 * Wait for the channel remote endpoint to connect.
 * Events:
 *   connect - emitted when the remote endpoint connects to the channel
...
Channel.prototype.listen = function() {
  ...
  var consumer = this.local || 'server';
  var producer = this.remote || 'client';
  this.qConsumer = new Queue(this.bus, 'channel:' + consumer + ':' + this.name);
  this.qProducer = new Queue(this.bus, 'channel:' + producer + ':' + this.name);
  this._attachQueues();
};

/**
 * Connect to the channel.
 *   connect - emitted when connected to the channel and the remote endpoint is ready to accept messages
 *   message - emitted when a message is received from the remote endpoint
 *   end - emitted when the remote endpoint disconnects from the channel
 */
Channel.prototype.connect = function() {
  ...
  var consumer = this.local || 'client';
  var producer = this.remote || 'server';
  this.qConsumer = new Queue(this.bus, 'channel:' + consumer + ':' + this.name);
  this.qProducer = new Queue(this.bus, 'channel:' + producer + ':' + this.name);
  this._attachQueues();
};
...
Channel.prototype.sendTo = function(endpoint, msg) {
  ...
  q.on('error', function(err) {
    _this._logger.error('error on channel sendTo ' + q.name + ': ' + err);
    q.detach();
  });
