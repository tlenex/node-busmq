...
Connection.prototype.connect = function(url) {
  ...
  for (var i = 0; i < this.numPhysical; ++i) {
    ...
    this._connect('main', i, _mainCommands, function(event, message) {
      emit(event, message);
    });
...
Connection.prototype._connect = function(type, index, commands, cb) {
  ...
  commands.forEach(function(c) {
    ...
    _this[c] = function() {
      ...
      // type will be 'main' or 'read'.
      // in effect, we will get something like: this['main'][2]['lrpop'] to invoke
      // the 'lrpop' command on a 'main' connection located at index 2
...
Connection.prototype.disconnect = function() {
  ...
  var _this = this;
  _this.main.forEach(function(c) {
    c.end();
  });
  _this.read.forEach(function(c) {
    c.end();
  });
  _this.emit('close');
}
