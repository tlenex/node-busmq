...
Connection.prototype._connect = function(type, commands, cb) {
  ...
  // create the client using the driver instance that was setup in the bus constructor
  var client = this.driver.create(this.port, this.host, this.auth);
...
Connection.prototype._setupListeners = function(client, type, cb) {
  ...
  var _onClientReady = function () {
    ...
    // slight variation on the server info variable between ioredis and node_redis....
    var serverInfo = client.serverInfo || client.server_info;
    if (serverInfo.role !== 'master') {
      clientLogger.isDebug() && clientLogger.debug('redis role is ' + serverInfo.role + ' but must be master. disconnecting and retrying connection in 1000ms');
      client.stream.end();
      return;
    }
