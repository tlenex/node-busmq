...
Connection.prototype._connect = function(type, index, commands, cb) {
  ...
  commands.forEach(function(c) {
    ...
    _this[c] = function() {
      ...
      _this.nextPhysical = ++_this.nextPhysical % _this[type].length;
      ...
      var sent = _this[type][_this.nextPhysical][c].apply(client, arguments);
      // if need to suspend sending on the connection
      if (sent === false && type === 'write') {
        // if we still have other physical connections available, it's ok
        if (++_this.suspended < _this[type].length) {
         sent = true;
        }
      }
      return sent;
...
Connection.prototype._setupListeners = function(client, type, cb) {
  ...
  client.on('drain', function () {
    if (type === 'write') {
      var emit = false;
      // if we were completely suspended, emit the drain event
      if (_this.suspended === _this[type].length) {
        emit = true;
      }
      _this.suspended = Math.max(--_this.suspended, 0);
      emit && active && cb && cb('drain');
    }
  });
