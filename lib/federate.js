...
Federate.prototype.to = function(ws) {
  ...
  var _onWsClose = function(code, message) {
    ...
    _onWsShutdown('closed');
  };

  var _onWsShutdown = function(reason) {
    _this.object.logger.debug('federation transport shutting down: ' + reason);
    ...
    if (_this.wsStream) {
      _this.wsStream.unpipe(this.dnode);
      _this.wsStream = null;
    }
...
Federate.prototype.close = function() {
  if (this.ws) {
    this.ws.emit('shutdown', 'requested close');
  }
};
