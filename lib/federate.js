 ...
 * @private
 */
Federate.prototype._sendEndFederation = function(cb) {
  var _this = this;
...
Federate.prototype._to = function(ws, reconnect) {
  ...
  var _onWsFederationEnd = function() {
    _this._sendEndFederation(function() {
...
Federate.prototype._federate = function(reconnect) {
  ...
  function callRemote(method, args) {
    // call the remote endpoint if it's an end federation request OR the federation hasn't closed or ended
    if (method === '_endFederation' || (!_this.closed && !_this.closing && !_this.endSent)) {
      _this.remote[method].call(_this.remote, args);
    }
  }

  // federate the methods to the remote endpoint.
  var methods = ['_endFederation'].concat(this.options.methods);
