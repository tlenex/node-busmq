...
FederationServer.prototype._onConnection = function(ws) {
  ...
  function onClose() {
    _this._endFederation('federate client connection closed', object, d, ws);
  }

  function onError(err) {
    _this._endFederation('federate client error: ' + JSON.stringify(err), object, d, ws);
  }
...
FederationServer.prototype._endFederation = function(msg, object, d, ws) {
  this.logger.info(msg);
  d.end();
  d._federationStream.unpipe(d);
  delete d._federationStream;
  object.detach && object.detach();
};
