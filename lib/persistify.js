...
exports = module.exports = function(bus, name, object, attributes) {
  ...
  object._pconnection = function() {
    ...
    if (!this._p.connection) {
      ...
      function _endOrClose() {
        _this._p.logger.debug("connection to persisted object terminated");
      }
      this._p.connection.on('end', _endOrClose);
      this._p.connection.on('close', _endOrClose);
      this._p.connection.on('drain', function() {
        _this.emit('drain');
      });
      this._p.connection.on('ready', function() { });
    }
  ...
  object.save = function(cb) {
    ...
    var _this = this;
    this.pwrite(this._p.persistKey, this._p.persistDirty, function(err, resp) {
      if (err || resp !== 'OK') {
      ...
      dirtyKeys.forEach(function(key) {
        _this._p.persistAttrs[key] = _this._p.persistDirty[key];
