...
Queue.prototype._emitAttached = function(exists) {
  ...
  this.ttl(function(err, ttl) {
    ...
    if (_this._state !== STATES.ATTACHING) {
      return;
    }

    _this._state = STATES.ATTACHED;
    _this._ttl = ttl;
...
Queue.prototype.push = function(message, cb) {
  ...
  return multi.exec(function(err, res) {
    if (err) {
      ...
      //BGR: we grab the messageId at the end of the transaction
      // IORedis will 'pipeline' the calls so intermediate results are not available in the chained functions callbacks
      // The format of results provided by the `exec` method differ slightly between IORedis and node-redis
      var messageId = _this.bus.useIORedis ? res[0][1] : res[0];
