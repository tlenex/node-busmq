...
Queue.prototype._emitAttached = function(exists) {
  ...
  this.ttl(function(ttl) {
    ...
    // we're attached, send all messages that were pushed when we were not yet attached
    _this._offlineQueue.forEach(function(msg) {
      _this.push(msg.message, msg.cb);
    });
    _this._offlineQueue = [];
...
Queue.prototype._touch = function(ttl) {
  ...
  this.qKeys.forEach(function(key) {
    _this.connection.expire(key, ttl, function(err, resp) {
      if (err) {
        _this.emit('error', "error setting key " + key + " expiration to " + ttl + ": " + err);
      }
    });
  })
};
...
Queue.prototype.push = function(message, cb) {
  if (!this.isAttached()) {
    // we're not attached yet, store the messages internally and push them all once we're attached
    this._offlineQueue.push({message: message, cb: cb});
    return false;
  }
...
Queue.prototype._consumeMessages = function() {
  ...
  function _take() {
    ...
    // if we are consuming and removing, use lpop
      _this.connection.lpop(_this.messagesKey, function(err, message) {
        _afterTake(err, message);
      });
    } else {
      // if we are consuming and not removing, use lindex
      _this.connection.lindex(_this.messagesKey, _this._consumeOptions.index, function(err, message) {
        ++_this._consumeOptions.index;
        _afterTake(err, message);
      });
    }

    function _afterTake(err, message) {
      if (err) {
...
Queue.prototype.consume = function(options) {
  ...
  if (!this.isAttached()) {
    var err = 'not attached';
    this.emit('error', "error consuming from queue: " + err);
    return;
  }

  this._consumeOptions = util._extend({remove: true}, options);
  ...
  if (!this._consumeOptions.remove) {
    this._consumeOptions.index = 0;
  }
