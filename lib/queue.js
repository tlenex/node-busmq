...
Queue.prototype._consumeMessages = function() {
  ...
  function _pop() {
    ...
    if (_this._consumeMax === 0) {
      delete _this._consumeMax;
      _this.stop();
      return;
    }
    ...
    _this.connection.lpop(_this.messagesKey, function(err, message) {
      ...
      if (message) {
        ++_this._consumed;
        _this._consumeMax && --_this._consumeMax;
 ...
 * @param max - the maximum number of messages to consume. if negative or undefined, will continuously consume messages as they become available.
 * Events:
 ...
 */
Queue.prototype.consume = function(max) {
  if (this.isConsuming()) {
    return;
  }
