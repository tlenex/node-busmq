...
function WSPool(bus, options) {
  ...
  this.logger.debug('websocket pool size set to ' + this.options.poolSize);
  ...
  this.logger.info('setting up ' + this.options.poolSize + ' websockets per pool for urls: ' + JSON.stringify(this.options.urls));
...
WSPool.prototype._add = function(url) {
  ...
  function onOpen() {
    _this.logger.debug('websocket to ' + url + ' added to pool');
  }

  function onClose() {
    _this.logger.debug('websocket to ' + url + ' closed');
  }
  ...
  function onReopen() {
    _this.logger.debug('websocket to ' + url + ' is reopened');
  }

  function onFatal(error) {
    _this.logger.error('websocket to ' + url + ' fatal error (placing url in error state): ' + JSON.stringify(error));
