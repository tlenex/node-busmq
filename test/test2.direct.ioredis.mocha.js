...
describe('BusMQ direct connectivity using ioredis', function() {

  var redisPorts = [10888,10889,10890,10891];
  ...
  describe('bus connection', function() {
    ...
    it('should emit online event when connected and offline event after disconnecting', function(done) {
      var bus = Bus.create({driver: 'ioredis', redis: redisUrls, logger: console, logLevel: 'debug'});
  ...
  describe('federation', function() {
    ...
    it('federates channel events', function(done) {
      var bus = Bus.create({driver: 'ioredis', redis: redisUrls, federate: {server: fedserver}, logger: console});
    ...
    it('does not allow federation with wrong secret', function(done) {
      var bus = Bus.create({driver: 'ioredis', redis: redisUrls, federate: {server: fedserver, secret: 'thisisit'}, logger: console});
