/**
 * This file runs the tests for a cluster layout
...
var RedisCluster = require('./helpers/redis-cluster');


var redisUrls = ['redis://127.0.0.1:7000', 'redis://127.0.0.1:7001', 'redis://127.0.0.1:7002'];

describe('BusMQ Cluster', function() {
  ...
  before(function(done) {
    redisCluster.start(done);
  });

  // stop all redis servers
  after(function(done) {
    redisCluster.stop(function() {
      done();
    });
  });
  ...
  describe('bus connection', function() {
    ...
    it('should emit online event when connected and offline event after disconnecting', function(done) {
      var bus = Bus.create({driver: 'ioredis', layout: 'cluster', redis: redisUrls, logger: console, logLevel: 'debug'});
    ...
    //   var bus = Bus.create({driver: 'ioredis', layout: 'cluster', redis: redisUrls, logger: console});
    //   tf.resumeSilently(bus,redisCluster, redisPorts[0], redisPorts[2], done);
    // });
  });
